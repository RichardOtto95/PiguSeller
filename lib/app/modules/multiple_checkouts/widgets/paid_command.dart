...
class PaidCommand extends StatelessWidget {
  final bottomMargin;
  final userName;
  final tableID;
  final orderID;
                  ...
                  if (!snapshot.hasData) {
                    ...
                    return Row(
                      children: [
                        PersonPhoto(
                          size: wXD(37, context),
                          avatar: snapUser.data['avatar'] != null
                              ? snapUser.data['avatar']
                            ...
                            children: [
                              ...
                              StreamBuilder(
                                stream: Firestore.instance
                                    ...
                                    .document(docId)
                                    .collection('orders')
                                    .where('item_status', isEqualTo: 'created')
                                    .snapshots(),
                                builder: (context, snapshot) {
                                  if (!snapshot.hasData) {
                                    return Container();
                                    ...
                                    double total = 0;
                                    // print(
                                    //     "snapshot.data.documents.first.reference ${snapshot.data.documents.first.reference}");
                                    snapshot.data.documents.forEach((element) {
                                      orders.add(element.data['ordered_value']);
                                      total += element.data['ordered_value']
                                          .toDouble();
                                    });
                                    // for (var i = 0;
                                    ...
                                    //     i++) {
                                    //   print(
                                    //       '##############3 vaaaaaaaalue anteeeeeeeeeeeees ${snapshot.data.documents[i].data['ordered_value']}###########');
                                    //   orders.add(snapshot.data.documents[i]
                                    //       .data['ordered_value']);
                                    //   print(
                                    //       '##############3 vaaaaaaaalue ${snapshot.data.documents[i].data['ordered_value']}###########');
                                    // }
                                    // for (num n in orders) {
                                    //   total += n;
                                    // }
